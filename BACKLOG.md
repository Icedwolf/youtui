...
- **Fake ffmpeg logs every invocation; the tests pin the exact relay count.** `fake_ffmpeg_body` appends each argv to a per-test log. Test A asserts exactly one `pipe:0` spawn — which also kills a silent coverage hole: on a no-ffmpeg host where the `check_ffmpeg` LazyLock is pre-initialized `false`, the pipeline would silently take the M4A path and test A would pass without exercising any throttle (now it fails loudly instead). Test B asserts exactly one `pipe:0` spawn — a regression that re-retried the throttled relay would spawn ffmpeg a second time and fail (previously it still produced the same `Err` and passed). Both assertions immune to the incidental `-version` probe line. Fail-first: disabling the log line fails both tests. Documented: the E2E PATH mutation is process-global and serialized only against each other; the sibling by-name spawners are skip-tolerant bench tests, so a stray overlap degrades benignly.
