...
- **`spawn_bg_cache_task` 9 positional args → `BgCacheTask` struct.** Removes the `#[allow(clippy::too_many_arguments)]`; the permit-is-held-until-fill-completes invariant is now named by the field (`permit`) rather than position. Two production + three test call sites updated.
- **Doc/hardening pass.** `STREAM_INIT_THRESHOLD = 512` gained the invariant comment (it sits under ffmpeg's ~700B atomic `ftyp+moov` first flush, so init always observes a complete `moov`; it is a wake-up gate, not a decodability guarantee). The M4A fallback branch documents that it deliberately carries **no** retry ladder — the throttle/client-fallback retries are ALAC-relay-specific (DECISIONS:40-42), so on a no-ffmpeg host a throttle/format-refusal skips the song; degraded by design and unreachable here (`check_ffmpeg` LazyLock always resolves true when ffmpeg is installed, and is process-global, so the M4A branch can't be E2E-tested on this host).
- Verified: 376 youtui bins green, clippy 0 warnings, `cargo build --release` clean.

### Session 2026-09-11 — Headless 1:1 latency measurement; relay first-byte log fixed

- **The phase-split claim is now proven with a real network run, no TUI needed.** A headless script reproduced the app's exact relay argv (`yt-dlp -f ba/bestaudio -o - --no-warnings --no-playlist --ignore-config --extractor-args 'youtube:skip=hls,translated_subs' --add-header "Cookie: <header>"` → ffmpeg `-i pipe:0` ALAC `frag_every_frame`), using the *same* domain-filtered 4739-byte header `extract_cookie_header_str` produces (a standalone replica compiled in /tmp, byte-identical to DECISIONS:34). Results for `NJea_zTBuik`: **spawn → yt-dlp first byte = 2.012s / 2.576s** (two runs); **yt-first-byte → ffmpeg moov (transcode + init-ready) = 0.006s**; decoder init sub-ms (debug373). So ~2.0–2.6s of the song start is yt-dlp Python bootstrap + resolve + CDN handshake; the entire in-app cost after the first byte is ~6ms. No further in-app latency lever exists without daemonizing yt-dlp (rejected: complexity). The earlier "~0.60s `--version` bootstrap ≈ 25%" refines to: it sits inside the 2.0–2.6s, not additive to it.
- **The relay's first-chunk log now measures the real quantity.** `relay: first chunk forwarded to ffmpeg` logged `buf_len = buffer_for_first.len()` — the *ffmpeg-output* buffer, which is empty by construction on yt-dlp's first input byte (≈0, misleading). Replaced with `first_chunk = n` (the actual forwarded chunk). Debug-only, zero test/behavior impact.
- **Settle docs corrected + helper folded.** A review caught that `mod.rs` still described the *disproven* live-download-only settle design — "every press after the first observes a live download and settles" — the exact overclaim the `last_download_trigger` fix was built to contradict (`drop_unscoped_from_id` drops the prior entry *before* the next decision). All four sites (`RESOLVE_SETTLE_MS` header, `DownloadConfig.settle_window_ms` doc, the `download_and_decode` settle comment) now describe the two-signal decision. `settle_window_ms(bool)` (single-caller bool→u64 wrapper) was folded into `Playlist::settle_window_for`, and its now-redundant test removed (subsumed by the 5 `fresh/recent/stale/live/tick` playlist tests). Net −~10 lines, one less indirection.
- **Measurement lesson (first runs):** the script's initial pipe-drain bug (never reading ffmpeg stdout) blocked the forward loop at ~64KB with zero output — the app avoids this by owning async stderr/stdout handlers on both children (`spawn_stdout_writer`/classifier). Not a code finding, but confirms the app's drain discipline is load-bearing.
- **The held-key storm needs no manual session**: the 1:1 simulation is the composition of GREEN tests — `burst_second_press_coalesces_after_out_of_scope_drop` (decision 100ms after drop, RED-proven), `cancel_download_during_settle_never_spawns_ytdlp` (cancelled mid-settle ⇒ 0 real spawns, fake-bin counter), `isolated_download_skips_settle_spawns_immediately` (first press 0ms ⇒ immediate spawn, <60ms), `settle_window_still_covers_key_autorepeat` (window > 33ms cadence). Every held-key outcome is bounded by the chain, and the fake-binary harness counts *real* spawn calls.
- Verified: 377 youtui bins green, clippy 0, `cargo build --release` clean.

### Session 2026-09-11 — Conditional settle; pipeline cleanup (retry-ladder + log dedup)

- **The 100ms settle is now conditional: an isolated selection pays zero, a burst in progress pays the full window.** A code review faulted the first conditional design (settle only when `active_downloads` is non-empty): `play_song` → `prepare_playback_id` → `drop_unscoped_from_id` cancels and removes the previous press's out-of-scope entry **before** `download_song`'s decision runs, so in a held-next burst every press past the first observes an empty set — the settle would have skipped for every press and the spawn-storm the window exists to prevent would return. The decision now reads two signals (`settle_window_for`, playback.rs): a live download (steady-play fill) **or** `last_download_trigger` — a per-Playlist `Instant` ticked on every `download_song` — within the window. An isolated selection with neither signal pays 0ms; any press within `RESOLVE_SETTLE_MS` of a prior one coalesces, exactly the pre-conditional behavior for bursts while keeping the single-song win. Fail-first proven: `burst_second_press_coalesces_after_out_of_scope_drop` reproduces the storm shape (press 1 registers + ticks, entry dropped, press 2 decides) — **RED** (settle 0) with the trigger signal neutered, GREEN with it. Decision is settled per-download (`DownloadConfig.settle_window_ms`), deterministically testable.
- **Pipeline cleanup: retry-ladder DRY, error-log dedup, first-byte instrumentation.** `try_pipeline_retry` helper unifies the three identical throttle+client-fallback blocks in the `'attempt` loop into one call site + decision-table test (`try_pipeline_retry_decision`). The error-handler bare `warn!("failing buffer")` after classifier warns now fires only for unclassified errors; throttle/format lines get `debug!`; dead/auth/novel keep their sole warn. A measurement-only `debug!("relay: first chunk forwarded to ffmpeg")` (buf_len + elapsed) splits spawn→first-byte (yt-dlp bootstrap + network) from first-byte→decoder-init (ffmpeg transcode) in live `--debug` logs. (Commits `7146ecb`, `0d47f3a`.)
...
- **The settle is no longer a mint guard — it's an extraction coalescer.** The original `RESOLVE_SETTLE_MS = 150` rationale was coalescing a held next/prev burst before it flooded the CDN with GVS token mints; the client flip (DECISIONS:42) removed the mint from the primary, leaving one **extraction per press** as the bot-signal residue. Window stays but shrinks to 100ms — above the ~30-40ms X11/Wayland key-autorepeat cadence (contract test `settle_window_still_covers_key_autorepeat` pins `> 50ms`), so a held key still coalesces to its landed song with no yt-dlp spawn. A deliberate <100ms mash leaks a token-free extraction killed on the next press (~ms of yt-dlp startup) — cheap next to the pre-flip mint flood. Saves **50ms per cold select** (first song and every distant-song select).
- **The one-time `ffmpeg -version` probe now runs at startup, not on the first song.** `check_ffmpeg` (LazyLock) was private and cold on the first download's critical path (~40-95ms host-measured spawn). Now `pub(crate)` and pre-warmed in `Server::new` — the first download's probe is a sub-ns cache hit, and the cost sits under the ~300ms autosave-queue load with zero added cold-start wall time. Combined with the settle: ~90-145ms off the first cold select.
- **Tests (fail-first proven):** `cancel_download_during_settle_never_spawns_ytdlp` (fake-bin yt-dlp counter file; cancel 60ms in; asserts `bail!` at the settle + 0 spawns — **RED with the settle neutered to 0ms** because the pipeline reaches the spawn and streams to Ok, GREEN at 100ms), `download_and_decode_pre_cancelled_bails_without_spawning` (pre-cancelled token bails at "before start", 0 spawns), `settle_window_still_covers_key_autorepeat` (const contract guard). Verified: 368 youtui bins green (+3), clippy 0 workspace, `cargo build --release` clean.
- Docs: DECISIONS:38 amended `[AMENDED BY 43]`, new DECISIONS:43. **Live measurement (debug373 vs debug372, `download_and_decode` → decoder-ready `elapsed=`): mean ~2.59s (range 2.10–3.41) vs ~3.03s (2.26–4.18) baseline — ~0.44s (~15%) mean, ~27% off the max.** `8VDgQDRfc8I` 3.33/3.78→2.89s, `NJea_zTBuik` 3.32→2.10s. Settle coalescing observed live (`download cancelled during settle cfg.video_id=wXQfwDmSo_w`); cache replays (`bkV-PBzXtPk`, `wXQfwDmSo_w`, `_UYhULU3Yb0`, `_NPBrtLBQdM`) played cleanly from full buffers.

### Session 2026-09-11 — Dead-video classifier matches yt-dlp's bare `Video unavailable`; trims reinstalled

- **`Come in Stranger` (Johnny Cash) always skipped with no message — and the reason was a classifier gap, not a playback bug.** `gPBZjyavHwc` is genuinely dead (fresh `yt-dlp`: `ERROR: [youtube] gPBZjyavHwc: Video unavailable`; oEmbed confirms the title), but `is_permanently_unavailable` required a suffix (`not available` / `no longer available` / `removed by the uploader`) that modern yt-dlp stopped printing — a bare `Video unavailable` fell through to generic `format not available (yt-dlp error)`: no dead-video notification, no session-skip, and the transient-failure halt counter (DECISIONS:23) got fed instead. Fix: match the bare phrase (`contains("video unavailable")`) — every historical suffix form is contained in it, and yt-dlp only prints it for the permanent class (removed/private/terminated/geo).
- **False-positive audit (user-mandated):** the classifier is `ERROR:`-line-gated (a warning can't trip it); `mark_dead_video` never fails the buffer (any ERROR line fails it anyway) so the widening only re-labels an already-doomed download, never changes a success; the dead-video downstream is already false-positive-tolerant (DECISIONS:22 — session-scoped `session_dead_videos`, no auto-remove, manual re-select retries, restart clears); the music extractor's verdict is terminal across all clients, so no fallback escape is lost. Geo-blocked (`...in your country`) moves from generic (would feed the halt counter) to dead — strictly better, still re-selectable after a VPN change.
- **Tests (fail-first):** `permanently_unavailable_classifier` gained the exact live repro line + the geo variant in `permanent` (RED on the old suffix-requirement — panicked on `gPBZjyavHwc: Video unavailable` — GREEN after). Transient rows (bot-check, cookies, `Requested format is not available`, 429, signed-in-only) unchanged-green — none contain the bare phrase, the web_music fallback classifier stays exclusive.
- **"Random sound pieces" on Suicidal Thoughts (Notorious B.I.G.): no code trace.** Log shows zero decode anomalies for every played song (one clean EOF per song; no DecodeError/seek/underflow/throttle). Concluded a one-off corrupt/glitchy source track, per the user's own assessment — not a regression. Song ID not identifiable; unresolved.
- **Reinstalled for testing:** fresh `target/release/youtui` (31,282,104B, 2026-09-10) replaced the active `~/.config/cargo/bin/youtui` (the `/usr/bin/youtui` Jan-2026 build is stale and unused). Verified: 368 youtui bins green, clippy 0 workspace, release clean. (**Committed with the flip + trims suite.**)

### Session 2026-09-11 — Start latency cut ~3.8×: primary/fallback client flipped to token-free defaults

- **The ~9.6s song start was the forced `web_music` GVS mint on every download.** Measured "first bytes" on `/usr/bin/yt-dlp` nightly@2026.08.30.232658 (user-upgraded past 2026.08.19 stable, which carried #17461 android_vr removal + #17462 web_embedded fallback): `web_music`+pot streamed its first byte at **9.39s** (≈ the user-perceived ~9.6s start), while yt-dlp's **default clients streamed itag-251 with no PO token and no nsig at 2.68s / 2.46s** — ~3.8× faster. Upstream removed the previously-default `android_vr` (#17461; dry-run on this host: `Requested format is not available`), and `web_embedded` resolves itag-251 but **fails to stream** — both dead as the old fallback target. The "fragile link" is now the **token client**, not the default path (also: ios dead, `--cookies` verified fixed-but-slower below).
- **Fix: flip primary/fallback.** The **primary** attempt now runs yt-dlp's default (token-free) clients — `apply_ytdlp_auth_args` drops the `--plugin-dirs`/`cli_path`/`player_client=web_music` forcing (provider present == no-provider branch, one hot path for both), and `relay_client_fallback_retry` re-runs the pipeline **once** through `web_music`+POT (~9.4s) only when the *default* clients report `Requested format is not available` and a provider is installed. `android_fallback` renamed `web_music_fallback` (resolve.rs, build/spawn_ytdlp, `web_music_fallback_used` in the pipeline); the `cfg.pot_provider.is_some()` gate is unchanged (provider = token source to escape to). Bounded: per-song one-shot, a web_music retry that also refuses bails as a generic transient (halt counter + DECISIONS:23 intact), no provider → guard refuses, never loops.
- **`--cookies` re-tested on the nightly (user chose the re-test): itag-251 `proto=https` present** — the old tv_downgraded m3u8-only regression that forced `--add-header` is fixed upstream — **but** resolves ~2× slower than the header (6.5s vs 2.9s, signed-in client probing), so `--add-header` stays for the hot path.
- **Tests (fail-first):** resolve.rs `pot_provider_keeps_primary_on_default_clients` (provider + primary → skip-only, no `--plugin-dirs`, no `player_client=web_music`) + `web_music_fallback_configures_plugin_cli_and_web_music` (fallback → plugin-dirs + cli_path + web_music + auth header); `no_pot_provider_skips_web_music`/decision/classifier tests unchanged-green (same boolean rows, flipped meaning). E2E `web_music_format_unavailable_falls_back_to_android_vr_and_plays` → `default_client_format_unavailable_falls_back_to_web_music_and_plays` (fake yt-dlp: default argv → format-unavailable, web_music argv → ALAC fixture; asserts 2 `pipe:0` spawns + frames); `format_unavailable_falls_back_only_once_then_bails` restructured comments only (same 2-spawns-then-bail shape). Fail-first proven: neutering the guard (`if false && ...`) turned the fallback E2E red with the exact `format not available (yt-dlp error)` skip.
- Docs: DECISIONS.md:41 marked superseded, new DECISIONS.md:42 (flip + measured numbers + `--cookies` finding; 30/31 amended), README PO-token section + Client-fallback bullet inverted, main.rs provider doc comment. Verified: 365 youtui bins green, clippy 0, `cargo build --release` clean. (**Still uncommitted**, with the 2026-09-10 cap-3 and 2026-09-11 suites — commit together.)

### Session 2026-09-11 — Client-scrape resilience: token-free `android_vr` fallback

- **A forced-`web_music` refusal skipped the song with no recourse — the video wasn't dead, the *client* was.** `Requested format is not available` (GVS token refused / SABR experiment stripping formats / abandoned client) was classified as a generic format failure: the pipeline bailed instantly because the retry ladder (`relay_throttle_retry`) is throttle-only. Verified on this host (dry-run matrix, s3YOGsSi0Oo): `android` was SABR-stripped *today* (#12482), `tv` is dead (#17389), `web_safari`/`web_embedded` are images-only without a JS runtime — while `android_vr` still returns itag-251 webm **with no PO token and no nsig**. The token client is the fragile link, so a token-free client is the escape hatch.
- **Fix: bounded client fallback.** New `is_format_unavailable_line` classifier → `SharedBuffer::mark_format_unavailable` (distinct from throttle/dead/auth); `relay_client_fallback_retry` re-runs the pipeline **once** with `player_client=android_vr`. `apply_ytdlp_auth_args` gained an `android_fallback` flag that drops the POT plugin/`web_music` args and forces the token-free client (verified current-client dry-run; documented in nvim-yt-player/VRChat-style client-fallback terms). Bounded: only fires while a provider forces `web_music`, one shot per song, and an android_vr retry that also refuses bails as a generic transient (halt counter + DECISIONS:23 intact). Never mints, never loops, and the cap-3 throttle ladder is untouched.
- **Tests (fail-first):** `format_unavailable_line_classifies`; `relay_client_fallback_retry_decision` (not-used→retry, used→refuse, no-provider→refuse, throttle/plain→never); E2E `web_music_format_unavailable_falls_back_to_android_vr_and_plays` (fake yt-dlp: web_music argv → format-unavailable error, android_vr argv → ALAC fixture; asserts 2 `pipe:0` spawns + frames); E2E `format_unavailable_falls_back_only_once_then_bails` (2 spawns then `format not available`). Fail-first proven: neutering the guard's `!android_fallback_used` condition turned both E2E tests red with the exact `format not available (yt-dlp error)` skip. resolve.rs `android_fallback_uses_tokenfree_client` asserts the argv (android_vr forced, no `--plugin-dirs`, auth header kept).
- **Probe-bug root cause documented (research correction):** the earlier standalone "Requested format is not available" refusals were the *probe's* wrong `--plugin-dirs` value (`.../bgutil-ytdlp-pot-provider/yt_dlp_plugins`), not a CDN wave — the app correctly passes `<config>/yt-dlp-plugins` (main.rs:484) whose direct child is the plugin package (yt-dlp's `candidate_plugin_paths` layout). With the corrected dir, `web_music` minted a GVS token and returned itag-251 with a real URL. Production argv verified correct; the new fallback is belt-and-suspenders for the client being SABR'd out from under the token (as `android` was today).
- Docs: new DECISIONS.md:41, README "Client fallback (bounded)" bullet. Verified: 365 youtui bins green, clippy 0, `cargo build --release` clean. (**Also pending uncommitted:** the 2026-09-10 cap-3 skip-fix suite — commit together.)

### Session 2026-09-10 — Throttle wave skipping songs fixed: relay cap 2 → 3

- **`xAbGAyd_-W4` in debug370 reproduced the recurring skip live, and it was the retry cap, not a token bug.** 11:15:42 relay attempt 1 → 403 (8.8s) → attempt 2 → **403 again (8.4s)** → `relay_throttle_retry` refused (`relay_attempts <= 1`) → `download failed while buffering, skipping` → queue advanced. The user re-selected 4s later: 403 → retry → **played**. Same song, fresh mint recovered — proving two throttles are never definitive. 8VDgQDRfc8I + 1MUbudqR_uE both survived on attempt 2 (~20s), and s3YOGsSi0Oo played first-try; the wave beats *two* consecutive mints often enough to skip songs.
- **Fix: `relay_throttle_retry` predicate `relay_attempts <= 1` → `<= 2`** (mod.rs:594) — a throttled relay gets up to two retries, three capped relay attempts total (relay → relay → relay). A third consecutive throttle IS definitive (bail as generic transient, halt counter + DECISIONS:23 intact, no endless loop; dead/auth never throttle, still fail fast). Cost: up to ~25s more per song during a wave where the first two mints are refused — a rare, user-visible tradeoff replacing a skip.
- **Tests (fail-first):** `relay_throttle_retry_decision` asserts (1)→retry, (2)→retry, (3)→bail; `throttle_relay_then_second_relay_recovers_and_plays` rewritten as `throttle_relay_twice_then_third_relay_recovers_and_plays` (fake 403→403→stream, asserts 3 `pipe:0` spawns + ALAC frames, the exact debug370 repro); `throttled_relay_failure_bails_after_capped_relay_retries` now asserts 3 spawns then `format not available` bail. All three were red on the old `<=1` predicate, green after.
- Docs: DECISIONS.md:39 amended (three attempts), new DECISIONS.md:40, README relay-retry bullet, mod.rs comments (581-585, 678-684; WARN now logs attempt N/3). The `--bypass-cache` plugin patch (getpot_bgutil_cli.py:146) verified intact — the recurring skips were NOT the token-cache bug.

### Session 2026-09-09 (worktree) — Big start delay root-caused; direct-URL path removed (always relay)

- **The reported >2s delay was a ~16s song start, and the cause was a double resolve.** debug365 (new cap build, `--debug`) for `qOwg1rlaWKM`: resolve spawn 15:56:55.488 → direct-URL throttle detected 15:57:01.860 → relay 15:57:01.864 → play started 15:57:11.580. The CDN throttled the shaped-but-still-ffmpeg direct fetch ~2/3 of the time, so each song paid the ~6.4s resolve cost **twice** (the throttled direct resolve + the relay's own resolve+download); ~12s of the ~16s start was the wasted first resolve.
- **Fix: the direct-URL path is removed — every download goes through the yt-dlp relay (path consolidation).** `download_and_decode` no longer calls `resolve_url`; it settles (`RESOLVE_SETTLE_MS = 150`, coalescing held-key storms before any spawn), takes the semaphore, and calls `ytdlp_pipeline`, which **always** spawns the credential-carrying yt-dlp relay (`-o -`) piped to ffmpeg `-i pipe:0` ALAC transcode (M4A full-download fallback without ffmpeg). One fresh token mint per song, never two. Removed with the path: `resolve_url`, the 6-hour URL cache (`url_cache_get/put/remove`, `URL_CACHE`), `throttled_url_retry`/`evict_cached_url`, `is_url_open_refusal`, the browser-shaped `FfmpegInput::Url` fetch (the `-user_agent`/`-referer`/`-headers` argv + `FFMPEG_USER_AGENT`), the `stream_url`/`from_url_cache` pipeline plumbing, and the startup warm-up (`first_song_to_preresolve`, `app.rs`). `resolve.rs` is now only the auth-arg helpers (`apply_ytdlp_auth_args`, `PotProvider`, `is_nonempty_cookie_file`, `file_has_auth_cookie`); `spawn_ffmpeg` degenerated to `(writer, label, video_id)` with a constant `-i pipe:0` argv; `spawn_ffmpeg_stderr_handler` is a pure logger (no throttle marking — the relay's yt-dlp stderr handler owns classification); `relay_throttle_retry` dropped its `from_url_cache` arg. The relay retry cap is two attempts total: relay → relay.
- **Test updates (fail-first):** test A rewritten as `relay_streams_alac_end_to_end` (relay `-o -` streams the ALAC fixture → decode, no resolve); tests B/C updated to assert relay→relay (2 `pipe:0` spawns; url_cache assertions removed); obsolete tests removed (`is_url_open_refusal_classifies`, `ffmpeg_stderr_403_marks_throttled`, `ffmpeg_stderr_url_open_refusal_marks_throttled`, `throttled_url_retry_decision`, the `url_input_*`/`pipe_input` `build_ffmpeg_command` tests, all `resolve.rs` URL-cache/outcome tests). Fake helpers simplified: `fake_ffmpeg_body` is a pure `cat` (writes its argv to a per-test log), `YTDLP_FAKE_HEAD` lost its `--print` branch. `FakePath` dirs disambiguated with an `AtomicUsize`.
- **Test-infra fix (deterministic flake):** the by-name-spawn bench tests (`download_pipeline_comparison`, `m4a_decoder_ttf_from_full_download`, `ffmpeg_relay_ttf_from_webm_file`) resolved the E2E fake `yt-dlp` from PATH and touched test B's `relay-count` counter file — so the first "relay" streamed instead of 403'ing (1 pipe:0 instead of 2, 3/3 parallel-suite failures). The bench tests now take the `PIPELINE_TEST_LOCK` (serialized against the PATH-mutating E2E tests); the pre-existing "self-heals on re-run" comment is superseded for these.
- Verified: 360 youtui bins green (3 consecutive clean parallel runs), workspace 635 passed/20 ignored, clippy 0 across the workspace, `cargo build --release` clean. DECISIONS.md:39 (28/29/34/35 marked superseded); README streaming/retry notes and the `--bypass-cache` rationale (no more URL cache) updated. (Worktree, uncommitted.)

### Session 2026-09-09 (worktree) — Held next/prev resolve storm triggered the 403 throttle

- **The user's held-key test reproduced a throttle and stopped playback.** debug364 (new cap build, `--debug`) shows the mechanism: a held next/prev key queued one download per press, and each download's `download_and_decode` → `resolve_url` spawned a yt-dlp `--print url` that minted a fresh PO token via `bgutil-pot` (~0.6s, bypass-cache). ~183 resolves spawned in ~3s (14:42:10–14:42:13), each then cancelled ~1–150ms later by the next press's `drop_unscoped_from_id` — but the yt-dlp spawn (and token mint) had already happened. A flood of token mints is a bot signal, which the CDN answers with 403 throttles; a throttled wave then 403s the direct-URL fetch *and* the relay, so playback stalls ("not playing anymore"). The prior "held next/prev resolve-stacking audited → closed, no change" (BACKLOG 2026-08-11) missed that the *token mint* happens in the spawn itself, before the cancellation reap.
- **Fix: a resolve settle window (`RESOLVE_SETTLE_MS = 150`, `resolve.rs`).** `resolve_url` now, after the URL-cache check and pre-spawn cancel check, `select!`s on the cancel token vs a 150ms sleep before `cmd.spawn()`. A press superseded within that window cancels here and never spawns yt-dlp (no token mint), so a burst coalesces to its final song — only the song the user lands on pays the token-mint + resolve cost. The single-song hot path pays the 150ms once (a small slice of the ~5s resolve+download); URL-cache hits and decoder-cache hits return before reaching it, and the warm-up resolve passes `None` (no settle).
- **Tests (fail-first):** `resolve_url_cancelled_during_settle_never_spawns` (cancel at 50ms → asserts yt-dlp was never spawned, pidfile absent; red without the settle, green with it). `resolve_url_cancelled_kills_child`'s cancel moved 150ms → 300ms so it keeps exercising the post-spawn `kill_on_drop` reap path (not the settle bail). Verified: 376 youtui bins green, clippy 0 across the workspace, `cargo build --release` clean. (Worktree, uncommitted.)

### Session 2026-09-09 (worktree) — URL-open refusal classified as a throttle; housekeeping

- **The CDN throttle's *first* signature was not classified → song skipped.** debug362 (new cap build) captured a real throttle: ffmpeg's direct-URL fetch was refused at *open* time — `Error opening input file https://…googlevideo.com/videoplayback?…&pot=…` — with **no** `403`/`forbidden` string, so `is_throttle_line` didn't match, `spawn_ffmpeg_stderr_handler` logged a bare WARN and did NOT `mark_throttled()`, and the empty-pipe bail (`source exited, empty pipe`) exited the `'attempt` loop **without** the relay retry. New `is_url_open_refusal` (`song_downloader/mod.rs`): `line.to_ascii_lowercase()` contains `"error opening input"` **and** `"http"` → treat as a throttle. Scoped by the URL content so a `pipe:0`/decode error never trips it. Fail-first: `is_url_open_refusal_classifies` + `ffmpeg_stderr_url_open_refusal_marks_throttled` (red when the `|| is_url_open_refusal` branch is removed, green with it). **[Note: machinery superseded by the direct-path removal in the entry above — `spawn_ffmpeg_stderr_handler` no longer marks throttles and the direct-URL fetch is gone.]**
- **Throttle/recovery events surfaced above `debug!` (diagnosability).** The running release binary logs at `WARN` unless `--debug`, so every throttle-and-recovery line was `debug!`-level and compiled out of the user's normal log — a relay 403 showed only `"failing buffer"`, never whether the capped retry then recovered the song or bailed. Four rare, user-significant retry events promoted to `warn!`: the two detection sites and the two retry decisions (`Stream URL throttled — retrying via relay`, `Relay throttled — retrying once with a fresh resolve (capped)`). A throttle wave now reads end-to-end at default level. **[Note: two of the four sites (the direct-URL detection + retry) were removed with the direct path; the remaining relay detection + `relay_throttle_retry` WARNs stay.]**
- **`video_id` threaded into both stderr handlers** (`spawn_stderr_handler`/`spawn_ffmpeg_stderr_handler` take an owned `String`), so every throttle/stderr/failing-buffer line carries the song's `videoId` — previously untraceable (the direct-URL path logged only the opaque CDN `id=o-…`). DECISIONS.md:36-37.
- **Housekeeping:** songsearch.rs `handle_text_entry_action` collapsed onto the shared message-dispatch macros (Submit/DeleteWord/_); the stale `// TODO: Streaming` in api.rs → accurate "first-page-only, no pagination" note; `// TODO: lazily loaded` in main.rs → rationale note; drawutils.rs theming TODO → "out of scope" note. Verified: 375 youtui bins green, clippy 0, release clean. (Worktree, accumulated into the direct-path commit.)

### Session 2026-09-08 — Background-cache replay 403 skips; ffmpeg anonymous-fetch fix

- **`bgutil-pot`'s on-disk token cache was replaying invalidated tokens ("some songs skip").** The plugin caches tokens in `~/.cache/bgutil-ytdlp-pot-provider/cache.json` (script mode hardcodes `Settings::default()`, no TTL). YouTube invalidates GVS tokens before their ~6h metadata expiry, so a cached token for a *replayed* song 403'd deterministically — on both the direct fetch AND the relay (which replays the same token), turning a transient throttle into a per-song failure. Repro: `NJea_zTBuik` 403'd with the cached token; after `rm cache.json` / `--bypass-cache` it played twice (rc=0). Fix is a **host-side** one-line patch to the installed plugin (`getpot_bgutil_cli.py`): make `--bypass-cache` unconditional so every resolve mints fresh (~0.6s; the app's 6h URL cache already dedups). yt-dlp hardcodes `bypass_cache=False`, so its own retry never helps. DECISIONS.md:33. Documented in README.
- **ffmpeg's direct-URL fetch was anonymous (the DECISIONS:28 "browser-shaped fetch" was a no-op).** In `build_ffmpeg_command` the `-user_agent`/`-referer`/`-headers` were placed **after** `-i <url>`; ffmpeg treats post-`-i` options as output options and silently drops them, so the fetch went out bare `Lavf/…` regardless of cookies (echo-server proof: no Cookie/Referer sent after `-i`). Moved **before** `-i`. `build_ffmpeg_command` also now needs a filtered header: the raw browser export is 9 KB over ffmpeg's request-header limit (`overlong headers`, EINVAL).
- **Cookie header domain-filtered + deduped (`server.rs`).** `extract_cookie_header_str` now keeps only `*.youtube.com`/`*.google.com` rows and drops duplicate names (an all-domain export carried unrelated ad cookies and a 9 KB header; duplicate `SID` across `.youtube.com`/`.google.com` confuses CDN session binding). Filtered header = 4739 bytes, safely under ffmpeg's limit while keeping SID/auth cookies. DECISIONS.md:34.
- **Fail-first:** `test_extract_cookie_header_netscape_preferred_over_cookie` encoded the old duplicate-name behavior (both values present); updated to assert the first (Netscape) occurrence wins and the trailing duplicate is dropped. Full suite 366 youtui tests green, clippy 0 across the workspace, `cargo build --release` clean. End-to-end: resolved `NJea_zTBuik` with a fresh token and fetched it via ffmpeg with the filtered header before `-i` → rc=0.
- **"Fails first, then plays" fixed: a throttled relay now gets one more attempt (3 capped total).** Evidence from the post-fix debug session (`debug360.log`) showed the residual class exactly: fresh GVS tokens intermittently 403 both the direct ffmpeg fetch *and* the relay (`SrqtW0fntag` skipped 16:42:18 with `format not available (yt-dlp error)`, played on manual re-select 14s later). The pipeline bailed after 2 attempts even though a third fresh resolve clearly plays. Now the yt-dlp stderr handler classifies `HTTP Error 403: Forbidden` as a throttle (`is_throttle_line`) on the relay too, and `relay_throttle_retry` (`mod.rs`) allows **exactly one** throttled-relay retry — three capped attempts: direct URL → relay → relay. Dead/auth failures never throttle, a second throttled relay still bails as a generic transient (halt counter intact), and the M4A path is untouched. Tests (fail-first): `relay_throttle_retry_decision`, `throttle_relay_then_second_relay_recovers_and_plays` (fake-binary E2E, asserts exactly 2 `pipe:0` spawns + ALAC frames), `throttled_relay_failure_bails_after_capped_relay_retries` (renamed, now asserts 2 then bail); `ffmpeg_403_line_classifies_throttle` gained the relay error line. Fail-first proven: disabling the gate turns both E2E tests red with the exact bug signature. DECISIONS.md:29. Full suite 368 youtui tests green, clippy 0, release clean. (Amended 2026-09-08.)

### Session 2026-09-04 — Replace custom PO-token machinery with yt-dlp POT provider

- **Custom generator removed.** Deleted `pot.rs`, its per-process token cache, Node subprocess, and `pot-provider/generate.mjs` wiring. `apply_ytdlp_auth_args` is now pure argv construction: with a detected provider it adds the plugin directory, `youtubepot-bgutilcli:cli_path`, and `web_music`; otherwise it retains the historical `skip=`-only route.
- **Hard provider pair.** Startup detects both `~/.config/youtui/yt-dlp-plugins/bgutil-ytdlp-pot-provider/yt_dlp_plugins/` and executable `~/.config/youtui/bin/bgutil-pot`; it never forces `web_music` with only one. Gate dry-runs: plugin+CLI loaded and returned a `pot=` URL that ffmpeg played; a missing CLI was reported unavailable, skipped `web_music`, and failed format selection rather than emitting an unusable URL. Cold token generation measured 772ms (170ms cached); full resolve measured 5.7s versus roughly 9s with the Node route.
- **Tests:** `pot_provider_configures_plugin_cli_and_web_music` and `no_pot_provider_skips_web_music` replace the generator-specific tests. README/DECISIONS document the host-side installation and the absence of a Node fallback.
...
- **Fake ffmpeg logs every invocation; the tests pin the exact relay count.** `fake_ffmpeg_body` appends each argv to a per-test log. Test A asserts exactly one `pipe:0` spawn — which also kills a silent coverage hole: on a no-ffmpeg host where the `check_ffmpeg` LazyLock is pre-initialized `false`, the pipeline would silently take the M4A path and test A would pass without exercising any throttle (now it fails loudly instead). Test B asserts exactly one `pipe:0` spawn — a regression that re-retried the throttled relay would spawn ffmpeg a second time and fail (previously it still produced the same `Err` and passed). Both assertions immune to the incidental `-version` probe line. Fail-first: disabling the log line fails both tests. Documented: the E2E PATH mutation is process-global and serialized only against each other; a sibling by-name spawn colliding with the ~10ms window degrades to a skip/garbage bench in the common case, but can panic `download_pipeline_comparison` (webm cat'd through untranscoded) or inflate the E2E pipe:0 count — both rare and accepted (serializing the 30s bench would tax every networked suite run; the flake self-heals).
