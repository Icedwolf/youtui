...
const CACHE_MAX_ENTRIES: usize = 3;
...
static BYTE_CACHE: LazyLock<Mutex<HashMap<String, Vec<u8>>>> =
    LazyLock::new(|| Mutex::new(HashMap::new()));
static CACHE_ORDER: LazyLock<Mutex<VecDeque<String>>> =
...
static URL_CACHE: LazyLock<Mutex<HashMap<String, String>>> =
    LazyLock::new(|| Mutex::new(HashMap::new()));
...
pub async fn resolve_url(video_id: &str, yt_dlp_cmd: &str) -> Option<String> {
    ...
    cache.insert(video_id.to_string(), url.clone());
    Some(url)
}
...
pub async fn download_and_decode(
    ...
    if let Some(stream_url) = cached_url {
        ...
        info!(%video_id, elapsed = ?t0.elapsed(), "URL-cache HIT — spawning ffmpeg direct");

        let mut ffmpeg = tokio::process::Command::new("ffmpeg");
        ffmpeg
            .args(["-i", &stream_url, "-fflags", "nobuffer", "-flags", "low_delay",
                "-f", "wav", "-loglevel", "error", "pipe:1"])
            .stdout(std::process::Stdio::piped())
            .stderr(std::process::Stdio::null())
            .kill_on_drop(true);
        let mut child = ffmpeg.spawn().map_err(|e| format!("spawn ffmpeg (url-cache): {e}"))?;
        let ffmpeg_stdout = child.stdout.take().ok_or("no ffmpeg stdout")?;
        ...
        let write_handle = tokio::spawn(async move {
            ...
            loop {
                match rdr.read(&mut buf).await {
                    ...
                    Err(e) => {
                        info!(error = %e, "ffmpeg stdout read error (url-cache), failing buffer");
                        writer.fail();
                        return;
                    }
                }
        ...
        while buffer.len() < 4096 && tokio::time::Instant::now() < deadline {
            tokio::time::sleep(std::time::Duration::from_millis(5)).await;
        }

        info!(%video_id, stream_type = "url-cache→ffmpeg→wav",
            buf_len = buffer.len(), elapsed = ?t0.elapsed(),
            "Trying early decoder init");
        match try_streaming_init(&buffer, None).await {
            Ok(decoder) => {
                info!(%video_id, buf_len = buffer.len(), elapsed = ?t0.elapsed(),
                    "Streaming decoder init succeeded (URL-cache)");

                let vid = video_id.to_string();
                let buf_for_cache = buffer.clone();
                tokio::spawn(async move {
                    match tokio::time::timeout(std::time::Duration::from_secs(120), write_handle).await {
                        Ok(Ok(())) => {}
                        Ok(Err(join_err)) => {
                            info!(%vid, error = %join_err, "ffmpeg writer panicked (url-cache)");
                            return;
                        }
                        Err(_) => {
                    ...
                    match status {
                        Ok(s) if !s.success() => {
                            info!(%vid, code = s.code().map_or("unknown".into(), |c| c.to_string()),
                                "ffmpeg (url-cache) exited with non-zero code");
                            return;
                        }
                        _ => {}
                    }
                    let data = buf_for_cache.data();
                    info!(%vid, len = data.len(), "Caching completed download (url-cache)");
                    cache_put(vid, data);
                });

                drop(_permit);
                info!(%video_id, elapsed = ?t0.elapsed(), "download_and_decode returning decoder (URL-cache)");
                return Ok(decoder);
            }
            Err(stream_err) => {
                info!(%video_id, error = %stream_err,
                    "Streaming decoder init failed (URL-cache), waiting for full download");

                match tokio::time::timeout(std::time::Duration::from_secs(120), write_handle).await {
                    Ok(Ok(())) => {}
                    Ok(Err(join_err)) => {
                        return Err(format!("ffmpeg writer panicked (url-cache): {join_err}"));
                    }
                    Err(_) => {
                        let _ = child.start_kill();
                        return Err("ffmpeg (url-cache) download timed out (120s)".to_string());
                    }
                }
                let status = child.wait().await.map_err(|e| format!("wait ffmpeg (url-cache): {e}"))?;
                if !status.success() {
                    let code = status.code().map_or("unknown".into(), |c| c.to_string());
                    return Err(format!("ffmpeg (url-cache) exited with code {code}"));
                }

                info!(%video_id, buf_len = buffer.len(),
                    "Creating decoder from completed download (URL-cache fallback)");
                let reader = buffer.reader();
                let source = ReadSeekSource::new(reader, None);
                let mss = MediaSourceStream::new(Box::new(source), Default::default());
                let d = SymphoniaDecoder::new(mss).map_err(|e| format!("decoder (url-cache fallback): {e}"))?;
                drop(_permit);
                return Ok(d);
            }
        }
    ...
    let skip = "dash,hls,translated_subs";
    ...
    info!(%video_id, elapsed = ?t0.elapsed(), "yt-dlp spawned");
    ...
    let (_stderr_handle, stdout_handle, mut child, _relay_handle) = if is_relay {
        ...
        let stderr_handle = tokio::spawn(async move {
            ...
            while let Ok(Some(line)) = lines.next_line().await {
                if let Some(bytes) = parse_total_size(&line) {
                    info!(total_bytes = bytes, "Parsed total size from yt-dlp progress");
                    buf_for_stderr.set_total_len(bytes);
                } else if line.contains("ERROR") {
                    warn!(stderr_line = %line.trim(), "yt-dlp stderr (error)");
                } else if line.contains("WARNING") {
                    info!(stderr_line = %line.trim(), "yt-dlp stderr (warning)");
                }
            }
            info!("yt-dlp stderr stream ended");
        });
        ...
        ffmpeg
            ...
            .stderr(std::process::Stdio::null())
            .kill_on_drop(true);
        let mut ffmpeg_child = ffmpeg.spawn().map_err(|e| format!("spawn ffmpeg: {e}"))?;
        let mut ffmpeg_stdin = ffmpeg_child.stdin.take().ok_or("no ffmpeg stdin")?;
        let ffmpeg_stdout = ffmpeg_child.stdout.take().ok_or("no ffmpeg stdout")?;
        ...
        let write_handle = tokio::spawn(async move {
            ...
            loop {
                match rdr.read(&mut buf).await {
                    ...
                    Ok(n) => writer.write(&buf[..n]),
                    Err(e) => {
                        info!(error = %e, "ffmpeg stdout read error, failing buffer");
                        writer.fail();
                        return;
                    }
                }
            }
            info!("ffmpeg mp3 stream ended, finishing buffer");
            writer.finish();
        });
        ...
        let write_handle = tokio::spawn(async move {
            ...
            loop {
                match rdr.read(&mut buf).await {
                    ...
                    Ok(n) => writer.write(&buf[..n]),
                    Err(e) => {
                        info!(error = %e, "yt-dlp stdout read error, failing buffer");
                        writer.fail();
                        return;
                    }
                }
            }
            info!("yt-dlp stdout stream ended, finishing buffer");
            writer.finish();
        });
    ...
    let (decoder, needs_cache) = if is_relay {
        // Wait for at least 4 KB of MP3 data in the buffer.
        let deadline = tokio::time::Instant::now() + std::time::Duration::from_secs(5);
        while buffer.len() < 2048 && tokio::time::Instant::now() < deadline {
        ...
        match try_streaming_init(&buffer, None).await {
            Ok(decoder) => {
                ...
                let _t0 = t0;
                tokio::spawn(async move {
                    match tokio::time::timeout(std::time::Duration::from_secs(120), stdout_handle).await {
                        Ok(Ok(())) => {}
                        Ok(Err(join_err)) => {
                            info!(%vid, error = %join_err, "ffmpeg writer task panicked");
                            return;
                        }
                        Err(elapsed) => {
                            info!(%vid, elapsed = ?elapsed, "ffmpeg download timed out (120s), killing");
                            let _ = child.start_kill();
                            return;
                        }
                    }
                    ...
                    match status {
                        ...
                        Ok(_) => info!(%vid, elapsed = ?_t0.elapsed(),
                            "ffmpeg completed successfully"),
                        Err(e) => {
                            info!(%vid, error = %e, "ffmpeg wait failed");
                            return;
                        }
                    }
                    let data = buf_for_cache.data();
                    info!(%vid, len = data.len(), elapsed = ?_t0.elapsed(),
            ...
            Err(stream_err) => {
                ...
                match tokio::time::timeout(std::time::Duration::from_secs(120), stdout_handle).await {
                    Ok(Ok(())) => {}
                    Ok(Err(join_err)) => {
                        return Err(format!("ffmpeg writer task panicked: {join_err}"));
                    }
                    Err(_elapsed) => {
                        let _ = child.start_kill();
                        return Err(format!("ffmpeg download timed out (120s)"));
                    }
                }
                ...
                info!(%video_id, "ffmpeg completed successfully");
                ...
                let source = ReadSeekSource::new(reader, None);
                let mss = MediaSourceStream::new(Box::new(source), Default::default());
                let d = SymphoniaDecoder::new(mss).map_err(|e| format!("decoder (fallback): {e}"))?;
                (d, true)
            }
        }
    } else {
        ...
        Ok(decoder) => {
            ...
            tokio::spawn(async move {
                match tokio::time::timeout(std::time::Duration::from_secs(120), stdout_handle).await {
                    Ok(Ok(())) => {}
                    Ok(Err(join_err)) => {
                        info!(%vid, error = %join_err, "{pipe_name} writer task panicked");
                        return;
                    }
                    Err(_elapsed) => {
                        info!(%vid, "{pipe_name} download timed out (120s), killing");
                        let _ = child.start_kill();
                        return;
                    }
                }
                let status = child.wait().await.map_err(|e| format!("wait {pipe_name}: {e}"));
                match status {
                    Ok(s) if !s.success() => {
                        info!(%vid, code = s.code().map_or("unknown".into(), |c| c.to_string()),
                    ...
                    Ok(_) => info!(%vid, "{pipe_name} completed successfully"),
                    Err(e) => {
                        info!(%vid, error = %e, "{pipe_name} wait failed");
                        return;
                    }
                }
                let data = buf_for_cache.data();
                info!(%vid, len = data.len(), "Caching completed download");
                cache_put(vid, data);
            });
        ...
        Err(stream_err) => {
            ...
            match tokio::time::timeout(std::time::Duration::from_secs(120), stdout_handle).await {
                Ok(Ok(())) => {}
                Ok(Err(join_err)) => {
                    return Err(format!("{pipe_name} writer task panicked: {join_err}"));
                }
                Err(_elapsed) => {
                    let _ = child.start_kill();
                    return Err(format!("{pipe_name} download timed out (120s)"));
                }
            }
            ...
            if !status.success() {
                let code = status.code().map_or("unknown".into(), |c| c.to_string());
                return Err(format!("{pipe_name} exited with code {code}"));
            }
            info!(%video_id, "{pipe_name} completed successfully");
    ...
    // Drop semaphore, cache if needed.
    drop(_permit);

    if needs_cache {
        let data = buffer.data();
        info!(%video_id, len = data.len(), "Caching completed download (fallback)");
        cache_put(video_id.to_string(), data);
    }

    info!(%video_id, elapsed = ?t0.elapsed(), "download_and_decode returning decoder");
    Ok(decoder)
}
...
/// on a blocking thread with a 5-second timeout.
///
/// `byte_len` is passed as-is to ReadSeekSource:
/// - `Some(total_size)` for M4A (isomp4 reader needs it for SeekFrom::End)
/// - `None` for MP3 (MPEG reader does not seek to end, no issue)
///
/// If the format reader tries to seek beyond available data (e.g.,
/// `seek(End(0))` in M4A's isomp4 probe), the inner Condvar blocks the
/// thread — `spawn_blocking` keeps this off the async runtime, and the
...
async fn try_streaming_init(
    ...
    let handle = tokio::task::spawn_blocking(move || {
        let source = ReadSeekSource::new(reader, blen);
        let mss = MediaSourceStream::new(Box::new(source), Default::default());
    ...
    match tokio::time::timeout(std::time::Duration::from_secs(5), inner).await {
        Ok(result) => result,
        Err(_elapsed) => {
            Err("decoder init timed out (format reader blocked on Condvar)".to_string())
        }
    }
}
...
mod tests {
    ...
    const TEST_MP3: &[u8] =
        include_bytes!("../../../../ytmapi-rs/test_json/test_upload.mp3");
    ...
    fn spawn_slow_write(buf: &Arc<SharedBuffer>, data: &[u8], chunk_delay: Duration) -> std::thread::JoinHandle<()> {
        let buf = buf.clone();
    ...
    fn create_decoder_from(buf: &Arc<SharedBuffer>) -> Result<SymphoniaDecoder, String> {
        let total = buf.total_len().unwrap_or(buf.len() as u64);
        let reader = buf.reader();
        let source = ReadSeekSource::new(reader, Some(total));
        let mss = MediaSourceStream::new(Box::new(source), Default::default());
        SymphoniaDecoder::new(mss).map_err(|e| format!("{e:?}"))
    }
    ...
    fn streaming_creates_decoder_with_full_data_incomplete_writer() {
        ...
        let mut dec = create_decoder_from(&buf)
            .expect("Decoder with all data written but writer alive");
    ...
    fn streaming_produces_frames_before_full_write_completes() {
        ...
        let full_write_estimate = (TEST_MP3.len().div_ceil(64 * 1024) as u64).saturating_sub(1) * 15;
        println!(
    ...
    fn full_download_requires_write_completion() {
        ...
        let mut dec = create_decoder_from(&buf)
            .expect("Decoder after full download");
        let ttf = time_to_first_frame(&mut dec, 44100);
        assert!(
            ttf.is_some(),
            "Full-download decoder should produce frames"
        );
    }
    ...
    fn streaming_cache_eviction_works() {
        for i in 0..5 {
            cache_put(format!("evict_test_{}", i), vec![0u8; 1024]);
        }
        // Cache max is 3, so only the last 3 should remain.
        assert!(
    ...
    fn start_ffmpeg_relay() -> (Arc<SharedBuffer>, std::thread::JoinHandle<()>, std::process::Child) {
        let buf = SharedBuffer::new();
        ...
        ffmpeg
            .args(["-i", "pipe:0", "-fflags", "nobuffer", "-flags", "low_delay",
                "-f", "mp3", "-compression_level", "5", "-ab", "128k",
                "-loglevel", "error", "pipe:1"])
            .stdin(std::process::Stdio::piped())
            .stdout(std::process::Stdio::piped())
            .stderr(std::process::Stdio::null());
        let mut child = ffmpeg.spawn().expect("spawn ffmpeg");
    ...
    fn ffmpeg_relay_ttf_from_webm_file() {
        ...
        let (buf, writer, mut child) = start_ffmpeg_relay();
    ...
    fn m4a_decoder_ttf_from_full_download() {
        ...
        match ttf {
            Some(dur) => println!(
                ...
                dl_dur, decoder_dur, dur, dl_dur + decoder_dur + dur, len,
            ),
    ...
    fn download_pipeline_comparison() {
        ...
        ffmpeg
            ...
            .stdout(std::process::Stdio::piped())
            .stderr(std::process::Stdio::null());
        let mut ff_child = ffmpeg.spawn().expect("spawn ffmpeg");
        let mut ff_stdin = ff_child.stdin.take().unwrap();
        let ff_stdout = ff_child.stdout.take().unwrap();
        ...
        let t_dec = Instant::now();
        let mut dec_relay = create_decoder_from(&relay_buf)
            .expect("decoder from relay stream");
        ...
        .stderr(std::process::Stdio::null())
        .status()
        .expect("spawn yt-dlp for M4A");
        let yt2_spawn_dur = t0.elapsed();

        let m4a_data = std::fs::read(output_path)
            .expect("M4A file should exist after yt-dlp");
        let _ = std::fs::remove_file(output_path);
        ...
        let t_dec = Instant::now();
        let mut dec_m4a = create_decoder_from(&m4a_buf)
            .expect("decoder from full M4A");
        ...
        // ── Results ──────────────────────────────────────────────────
        let relay_playable = relay_data_arrival
            + relay_decoder_dur
            + relay_ttf.unwrap_or_default();
        let m4a_playable = yt2_spawn_dur
            + m4a_decoder_dur
            + m4a_ttf.unwrap_or_default();
        let relay_dl_fraction = relay_data_arrival.as_secs_f64()
            / relay_total_dur.as_secs_f64().max(0.001);
        let m4a_dl_fraction = m4a_dl_dur.as_secs_f64()
            / m4a_total_dur.as_secs_f64().max(0.001);
        ...
        println!(
            ...
            yt_spawn_dur, ff_spawn_dur,
            relay_buf.len(), relay_data_arrival,
            relay_decoder_dur,
            relay_ttf.unwrap_or_default(),
            relay_playable,
            relay_total_dur,
            relay_buf.len(), relay_dl_fraction * 100.0,
            yt2_spawn_dur,
            m4a_decoder_dur,
            m4a_ttf.unwrap_or_default(),
            m4a_playable,
            m4a_total_dur,
            m4a_buf.len(), m4a_dl_fraction * 100.0,
            relay_playable,
            m4a_playable,
    ...
    fn wav_decoder_from_file() {
        ...
        if !wav_path.exists() {
            eprintln!("SKIP: /tmp/test_output.wav not found (run: ffmpeg -i test.mp3 -f wav /tmp/test_output.wav)");
            return;
        }
