...
30. **YouTube Music now requires a GVS PO token on the `web_music` client, bound to each *video ID*.** A static `po_token.txt` is obsolete: current yt-dlp rejects a bare token (it needs `CLIENT.CONTEXT+TOKEN`) and a token not bound to the resolved song is 403'd by the CDN even with `pot` attached (verified: a visitor_data-bound token played one song and 403'd a second). youtui auto-mints a per-video token (`node <config>/pot-provider/generate.mjs -c <video_id>`) at resolve time and forces `player_client=web_music;po_token=web_music.gvs+<token>`, which returns a WEB_REMIX itag-251 URL that ffmpeg plays. nsig solving alone (yt-dlp-ejs) does **not** fix this — a no-pot WEB/ANDROID_VR URL still 403s. (Session 2026-09-04.)
31. **The web_music client is forced only when a token exists.** `apply_ytdlp_auth_args` emits `player_client=web_music` only when node + the generator are present and produced a token; otherwise it emits `skip=` alone (the historical ANDROID_VR/M4A path), preserving a no-node/no-generator host. Without a pot, `web_music` errors out, so forcing it unconditionally would be a regression.
